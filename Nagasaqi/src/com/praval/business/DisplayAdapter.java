...
public class DisplayAdapter extends JFrame{
	...
	public DisplayAdapter() {
		...
		setTitle("Display Adapter.");
 		...
 		lblDisplay.setText("NagaSaqi Synth:" + "                                                                                                " +
 				"Controller"+"::");
 		...
 		contentPane.add(txtOctave);contentPane.add(txtRoot);
 		
 		InstrumentChooser ic = new InstrumentChooser(SimpleMIDIAdapter.getInstrumentList(),this);
 		ic.setEnabled(true);
 		...
 		for(int i=0;i<7;i++){
 			...
 			jb.setText(ScaleBase.identifyNoteFromNumber(new ScaleBase().listScales.get(0).frequencies[i]));
 			jb.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent arg0) {
					...
					for(JButton jb:btnNotes){
						if(
								!(btnNotes.indexOf(jb) > btnNotes.indexOf(this))
						){
							jb.setEnabled(true);
					}
	...
	public void refreshTuning(int[] freq){
		...
		for(int x:freq){
			...
			System.out.println("N" + count + "_" + instrumentAdapter.getScaleBase().identifyNoteFromNumber(x));
			
			btnNotes.get(count++).setText(instrumentAdapter.getScaleBase().identifyNoteFromNumber(x));
		}
