...
public class InstrumentAdapter extends JPanel  implements MouseInputListener, MouseWheelListener{
    ...
    public InstrumentAdapter() {
       ...
       //Register for mouse-wheel events on the text area.
    	...
    	simpleMIDIAdapter = SimpleMIDIAdapter.getInstanceOf(channel, volume, duration);
	...
	public void mousePressed(MouseEvent e) {
		...
		switch(e.getButton()){
		case 1:
			pressMemory.set(0, !pressMemory.get(0));break;
		case 2:
			pressMemory.set(1,  !pressMemory.get(1));break;
		case 3:
			pressMemory.set(2,  !pressMemory.get(2));break;
		}
		printpressMemory();
	...
	private synchronized void play() {
		...
		if(diff < 80){
			...
			//SimpleMIDIAdapter.noteOff(scaleBase.getListScales()
					//.get(display.getScalenum()).frequencies[mapInput(getSingleNoteNumber(pressHistory.getModifiedState()))], 80);
			SimpleMIDIAdapter.noteOn(scaleBase.getListScales()
					.get(display.getScalenum()).frequencies[mapInput(getSingleNoteNumber(pressMemory))], 80);
			
			//color magic
			
			display.btnNotes.get(mapInput(getSingleNoteNumber(pressMemory))).setEnabled(false);
			System.out.println(mapInput(getSingleNoteNumber(pressMemory)) + "," + scaleBase.getListScales()
					.get(display.getScalenum()).frequencies[mapInput(getSingleNoteNumber(pressMemory))]);
		}else{
			//SimpleMIDIAdapter.allNotesOff();
			SimpleMIDIAdapter.noteOn(scaleBase.getListScales()
					.get(display.getScalenum()).frequencies[mapInput(getSingleNoteNumber((pressHistory.getMostRecentState())))], 80);
			//color magic
			display.btnNotes.get(mapInput(getSingleNoteNumber((pressHistory.getModifiedState())))).setEnabled(false);

			System.out.println(mapInput(getSingleNoteNumber((pressHistory.getMostRecentState()))) + ","+ scaleBase.getListScales()
	...
	public void mouseReleased(MouseEvent e) {
		...
		switch(e.getButton()){
		case 1:
			pressMemory.set(0, false);break;
		case 2:
			pressMemory.set(1, false);break;
		case 3:
			pressMemory.set(2,  false);break;
		}
		printpressMemory();
		...
		if(this.isAnyButtonDown()){
			...
			if(e.getWheelRotation()<0){
				scaleBase.getListScales().get(display.getScalenum()).frequencies=
						shiftarray(scaleBase.getListScales().get(display.getScalenum()).frequencies, -1);
	...
	private void shiftList(ArrayList<JButton> btnListBend, int wheelRotation,boolean resetFLag) {
		// TODO Auto-generated method stub
		for(JButton j:btnListBend){
		if(!resetFLag){
				j.setText(Integer.toString(Integer.parseInt(j.getText()) + wheelRotation));
		}
		else{
	...
	public void refreshDataKeyScale(){
		...
		display.txtRoot.setText((String)display.cboKey.getSelectedItem()+ octave);
		display.refreshTuning(scaleBase.getListScales().get(display.getScalenum()).frequencies);
	}
