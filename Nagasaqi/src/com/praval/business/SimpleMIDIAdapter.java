...
public class SimpleMIDIAdapter {
	...
	int flute = 79;//test instrument for prgram change aka instrument
	private SimpleMIDIAdapter(int channel,int volume, int duration) {
		...
		try{
			...
			channels[0].programChange(ins[0].getPatch().getBank(),ins[0].getPatch().getProgram());
	...
	public static SimpleMIDIAdapter getInstanceOf(int channel,int volume, int duration){
	...
	public static void noteSlide(int amount){
		...
		System.out.println("SET Pitch Bend:"+ (bend) + "," + channels[channel].getPitchBend());
	}
	...
	public static void setInstrument(int instrument) {
		...
		Instrument[] ins=synth.getLoadedInstruments();
		synth.getChannels()[0].programChange(ins[instrument].getPatch().getBank(),ins[instrument].getPatch().getProgram());
